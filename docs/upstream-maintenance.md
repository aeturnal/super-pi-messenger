...
`upstream` is a read-only source for optional maintenance, not a release dependency. The upstream push URL must remain `DISABLED`. Pi Super Messenger `main` never tracks `upstream/main`; upstream changes never merge automatically, and product progress never waits for upstream acceptance or releases.
